...
**Now anchor it to a governed definition.** In your Genie Agent → **Genie Code** → **trusted assets**, add the metric view `pcp_continuity_metrics` and the function `pcp_continuity_ratio`. Ask the continuity question again, any phrasing, and **Show generated code**. Now the answer **anchors to the governed definition** — often Genie calls `MEASURE(\`PCP Continuity Ratio\`)` / the function by name (a **"Trusted"** badge signals it used a trusted asset); other times it hand-writes SQL, but *with* the governed rule because the definition is in scope. Either way it lands on **76.3%**, not the naive 65.5%.
