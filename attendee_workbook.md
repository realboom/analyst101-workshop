...
- **Option C · Short on time? Do it in a notebook (code).** If the visual build won't fit the session — or an analyst prefers code — open **`notebooks/01_etl_notebook_alternative.py`**, attach **serverless**, set the widgets to your `catalog`, your `schema`, and the `profile` you uploaded, and run its **three cells**: (1) read and **explore** the CSV, (2) query it with **`%sql`**, (3) **Spark SQL** to clean it into `dim_facility`. Same raw file in your `landing` volume, same bronze → silver → gold logic, same **12-row** result — just code instead of the visual canvas. It writes the identical `dim_facility`, so **Steps 5–6 below apply either way.**
